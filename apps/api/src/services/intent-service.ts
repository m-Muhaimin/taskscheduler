...
function isReschedule(s: string): boolean {
  ...
  const reschedulePhrases = [
    ...
    'moving my booking',
    'shifting my appointment',
    'shifting my booking',
  ];
  if (reschedulePhrases.includes(s)) {
    return true;
  }
  // Starts-with variants (e.g. "reschedule please").
  if (
...
function isConfirm(s: string): boolean {
  ...
  if (confirmPhrases.includes(s)) {
    return true;
  }
  if (s.startsWith('confirm')) {
    return true;
  }
  if (s.startsWith('book it') || s.startsWith('book me')) {
    return true;
  }
  return false;
}
...
/** True when `normalized` looks like a bare slot-number choice (1, 2, 3, "option 2", "choice 3"). */
function looksLikeSlotChoice(s: string): boolean {
  // Bare integer 1-9 (we only ever offer 3 slots, but be generous).
  if (/^\d+$/.test(s)) {
    const n = Number(s);
    return Number.isInteger(n) && n >= 1 && n <= 9;
  }
  // "option N", "choice N", "pick N", "i choose N", "my choice is N".
  if (/^(option|choice|pick|i choose|my choice is|select)\s*\d+$/.test(s)) {
    return true;
  }
  // "number N", "slot N".
  if (/^(number|slot)\s*\d+$/.test(s)) {
    return true;
  }
  return false;
}
 ...
 *
 * `conversationStateExists` is true when a `ConversationState` row exists
 * for the sender's phone. Slot-choice only fires when both the message
 * looks numeric AND state exists; bare numbers without state → unknown.
 */
